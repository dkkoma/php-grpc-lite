...
- regression baseline は `bench/baselines/regression.json`。明示的な性能変化を受け入れる時だけ更新し、通常は `BENCH_BASELINE=bench/baselines/regression.json ./bench/run.sh cold` のように比較する。
