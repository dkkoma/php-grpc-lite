...
- call abandonment後のconnection reuseはconnection/session-scopedなreceive byte-boundary stateで決める。nghttp2が受理したraw bytesから9-byte frame header、frame payload残量、HEADERS / CONTINUATION block継続を追跡し、receive sideがclean frame boundaryかつheader block非継続の場合だけ再利用する。
...
ただしdeadline、明示cancel、resource destructor、stream-local semantic-error teardownでcallを放棄した直後のreuse admissionには、より強いreceive-side invariantを適用する。HTTP/2 connectionは `grpc_h2_receive_boundary_state` をsession lifetimeで保持し、各 `nghttp2_session_mem_recv()` が正常にconsumeしたraw bytesを `grpc_h2_receive_boundary_state_consume()` へ同じ長さだけ渡す。9-byte frame headerの一部、任意frameのpayload残量、またはHEADERS / CONTINUATIONにまたがるheader block継続のいずれかがあればclean frame boundaryではないため、`grpc_h2_receive_allows_reuse_after_abandonment()` はfalseとなる。trueとなるのはframe header / payloadがともに完了し、header blockも完了している場合だけである。partial HEADERS payloadのように `on_begin_headers_callback()` とsemantic phase開始へまだ到達していない状態も、このbyte-level stateからfail closedになる。

このreuse判定はHTTP/2 Connection / diagnostic session scopeが唯一のownerである。call-local `response_header_phase` はinformational / final initial / trailingのfield semanticsだけを所有し、open phaseは低層stateでもheader block継続となる一例にすぎない。productionはfalseを既存のclose-after-pending-flush actionへ写像し、targetのprimary statusとcaller-selected RST codeを維持したままconnectionをdead化する。raw diagnosticは同じtracker / predicateをiteration reset外のsession stateとして共有し、one-shot fdを有限に終了する。normal receive、persistent preflight drain、diagnostic poll / blocking receiveの全 `nghttp2_session_mem_recv()` 経路で同じtrackerを更新し、callback層ごとのreuse markerは追加しない。
...
HTTP/2 resourceの所有権はC extension側に閉じる。`grpc_call` per-call stateが持つbody buffer、queued payload、metadata、`grpc-message` はcall完了・failure・resource destructorのいずれでも同じcleanup pathを通す。明示cancel時と未完了server streaming resource destructorでは、対象streamへ `RST_STREAM(CANCEL)` を送る。stream cancelは原則connection failureではないが、abandonment時のreceive byte-boundary predicateがfalseならconnection-global parser stateを次callへ持ち越さずterminal化する。
...
互換性・制御系は `docs/verification/compatibility-control-checklist.md` に沿って追加する。HTTP/2 surfaceではTLS/mTLS、metadata/status/compression/error semanticsの代表条件を検証済み。実装進捗や検証途中の制限は `docs/research/` に残し、このdesign docには最終設計として満たすべき形だけを書く。
