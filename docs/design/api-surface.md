...
- [x] `Grpc\Timeval` — 薄い値オブジェクトとして extension 側で実装済み(`src/surface.c`、`tests/phpt/003-timeval.phpt`)
...
- `Grpc\Timeval` はレガシーユーザーコード互換のため **薄実装で提供** に分類変更(§2.3 補足。SPEC.md §4.5 と一致)
