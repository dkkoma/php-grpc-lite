...
| `on_header_callback()` | 全fieldのwire budget計上、`:status`でのphase確定、valid blockのstatus/metadata/content-type分類 | wire budget超過、またはEND_STREAMなしFINAL_INITIAL status field観測時に `RST_STREAM(CANCEL)`。後者がEND_HEADERS未完了ならconnectionへclose-after-pending-flush terminal quarantineを設定 |
