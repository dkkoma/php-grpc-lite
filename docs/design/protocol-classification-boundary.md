...
| Receive boundary after call abandonment | connection/session-scoped `grpc_h2_receive_boundary_state`（partial frame header、payload残量、HEADERS / CONTINUATION block継続）、connection / diagnostic sessionのterminal marker | 全 `nghttp2_session_mem_recv()` consumer、`grpc_h2_receive_allows_reuse_after_abandonment()`、production `cancel_grpc_call_stream()` / callback-owned semantic RST helper / inactive persistent preflight、raw diagnostic teardown | deadline / explicit cancel / stream-local semantic errorのstatusへ直接写像しない。clean frame boundaryかつheader block非継続の場合だけreuseを許可し、それ以外はcall-local owner消失前またはinactive connectionの再admit前にconnection lifecycleへterminal actionをhandoffする。semantic status/detailsとselected RST codeは維持する。phaseはfield semanticsだけを所有し、このreuse gateと並列に判定しない。fatal I/O / nghttp2 errorはpredicateと独立にconnection deadを所有する |
...
| mark connection dead | connection lifecycle | socket EOF/error、TLS fatal error、nghttp2 session error、stream registration failure、call abandonment / callback-owned semantic RST / inactive preflightでincomplete receive boundaryを検知しpending control frameをflushした後 | persistent cacheから外す対象。partial frame header / payloadまたはHEADERS / CONTINUATION block継続が残るconnectionはRST ownerにかかわらず全ownerにterminalで、対象call以外の既存siblingも追加I/Oなしでconnection failureへ収束する |
...
|---|---|---|
| production / diagnosticの全 `nghttp2_session_mem_recv()` consumer | 正常consumeしたraw bytesをconnection/session-scoped `grpc_h2_receive_boundary_state`へ反映し、partial frame header / payloadとHEADERS / CONTINUATION block継続をcallbackより下で追跡 | なし。reuse actionはcall abandonment、callback-owned semantic RST、inactive persistent preflightの各seamがshared pure predicateから決める |
...
| `cancel_grpc_call_stream()` / raw diagnostic teardown | connection/session receive stateがclean frame boundaryかつheader block非継続かをshared pure predicateで判定。field-class / phase routeは再実行しない | predicateがfalseならtarget taxonomy / selected RSTを維持したままconnection-terminal actionへhandoffし、productionはbounded control flush後にconnectionをdead化する。raw diagnosticはsession-terminal markerをsetし、timeoutならexact CANCELをnonblockingでbest-effort flushして有限終了する。fatal error後のunregister / owner clearはdead connection上のbookkeeping |
| `grpc_protocol_submit_rst_stream_in_callback()` | callback内のstream-local semantic failureでRSTをsubmitする直前にshared receive-boundary predicateを判定。callerが選択したstatus / details / RST codeは再分類しない | predicateがfalseならconnection close-after-pending-flushをmarkしてから、選択済みRSTをsubmitする |
| `preflight_persistent_connection()` | inactive persistent connectionのsocket/TLS preflightとlate-byte drain後にshared receive-boundary predicateを再判定 | predicateがfalseならconnectionをdead化し、cacheから外してfresh connectionへfallbackする。active ownerを持つmultiplexed connectionにはinactive admission gateを適用しない |
