...
| Response header protocol error | `response_header_protocol_error` | `grpc_protocol_reject_response_regular_header_before_status()`、invalid-frame callback、final前DATAに対するoutbound protocol-RST observer、stream close fallback | `INTERNAL`。`grpc-status` / HTTP status fallbackより優先 |
...
| mark connection dead | connection lifecycle | socket EOF/error、TLS fatal error、nghttp2 session error、stream registration failure、END_HEADERS未完了blockでpending CANCELをflushした後 | persistent cacheから外す対象。未完了inbound HPACK blockでは全ownerにterminalで、対象call以外の既存siblingも追加I/Oなしでconnection failureへ収束する |
...
| `on_header_callback()` / `on_invalid_header_callback()` | 全fieldのwire budget計上、`:status`でのphase確定、regular-before-`:status` の早期protocol classification、valid blockのstatus/metadata/content-type分類 | regular-before-`:status` は `RST_STREAM(PROTOCOL_ERROR)`、wire budget超過またはEND_STREAMなしFINAL_INITIAL status fieldは `RST_STREAM(CANCEL)`。terminal classificationがEND_HEADERS未完了なら共通close-after-pending-flush actionを設定 |
...
| malformed response header sequence | `response_header_protocol_error` | complete blockはnghttp2 rejectionを観測。END_HEADERS未完了のregular-before-`:status`はcallback時点で `RST_STREAM(PROTOCOL_ERROR)` をsubmitし、flush後にconnectionをdead化 | complete blockはconnection usableならreusable。未完了blockはterminalでfollow-upはfresh | `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_status_core.c` |
