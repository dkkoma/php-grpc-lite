...
| Invalid gRPC status field | `invalid_grpc_status`, `grpc_status_seen`, `initial_grpc_status_seen`, `initial_headers_end_stream` | `on_header_callback()`, shared terminal-initial frame-end helper | `UNKNOWN` |
...
| `on_frame_recv_callback()` | GOAWAY refusal、RST_STREAM、missing content-type、Trailers-Only misuse、PUSH_PROMISE拒否、phase endの分類 | GOAWAYでdraining化、PUSH_PROMISEで `RST_STREAM(PROTOCOL_ERROR)`。END_STREAMなしfinal initial status fieldはshared helperで `RST_STREAM(CANCEL)` |
...
| status fieldがEND_STREAMなしfinal initial blockにある、または`grpc-status` valueがinvalid | `invalid_grpc_status` | non-terminal final initialはframe-endで即時 `RST_STREAM(CANCEL)`。value parse failureはstatus taxonomyへ反映 | reusable if connection usable | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_protocol_core.c`, `tests/unit/test_status_core.c` |
...
| response metadata / wire header budget too large | `metadata_too_large`。final HTTP status前でも `RESOURCE_EXHAUSTED` とheader/metadata budget専用details | `RST_STREAM(CANCEL)` | reusable if connection usable | `tests/phpt/025-resource-limits.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/Integration/MetadataCompatibilityTest.php` |
