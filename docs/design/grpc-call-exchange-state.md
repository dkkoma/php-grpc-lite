...
| response header-block semantic phase | `response_header_phase`, `response_header_block_end_stream`, `response_header_block_protocol_valid` | `response_header_phase.c` のpure helperがbegin / `:status` / end / call resetの遷移を所有し、production / diagnostic callbackが共有する。END_STREAM validityはblock-local stateとしてsemantic fieldのcommit gateに使う | header hot path |
