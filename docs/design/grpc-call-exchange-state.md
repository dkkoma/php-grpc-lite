...
|---|---|---|---|
| bench専用の観測 | `fd`, `bytes_sent`, `bytes_received`, `data_read_calls`, `data_recv_calls`, `last_session_error`, frame counter群, `bench` | `PHP_GRPC_LITE_ENABLE_BENCH` buildだけ。diagnostic sessionの `grpc_h2_receive_boundary_state` とterminal markerはiteration reset対象外で、productionと同じpure byte tracker / reuse predicateを共有する。status / field semanticsのownerではない | diagnostic only |
...
既にclose / unregisterされたstream、またはforeign streamのHEADERSはlive `grpc_call`を持たず、上のfield notification routing tableへ入らない。これはfield classや`NONE` phaseではなくconnection/session lifecycleである。production / diagnosticの`on_begin_frame`はshared pure predicateでEND_HEADERS未完了のunowned HEADERSを検知し、call taxonomyやstream RSTを変更せずconnection-terminal markerを設定する。このcallback-level actionとは別に、connection/sessionは全receive bytesを `grpc_h2_receive_boundary_state` へ累積し、frame header / payload / header blockの完了状態をcallbackより下で保持する。

deadline、明示cancel、server streaming resource destructorでlive callを放棄するときは、field notificationではないため上表へsynthetic routeを追加しない。これらが収束する `cancel_grpc_call_stream()` は、semantic phaseではなく `grpc_h2_receive_allows_reuse_after_abandonment()` をreuse gateとして確認する。callback内でstream-local semantic errorを確定してRSTをsubmitする経路は `grpc_protocol_submit_rst_stream_in_callback()` で同じpredicateを確認し、inactive persistent connectionを再admitする経路は `preflight_persistent_connection()` がsocket/TLS drain後に同じpredicateを再確認する。この3つのenforcement seamにより、owner解放時、callback-owned RST時、late bytesをdrainした再admit時のいずれもbyte-level invariantを迂回しない。9-byte frame headerの一部、任意frame payloadの残り、またはHEADERS / CONTINUATION block継続があればfalseとなり、`on_begin_headers_callback()` 前のpartial HEADERS payloadも、phase開始後のfragmented blockも同じconnection-terminal handoffへ収束する。open phaseはこの低層header-block継続の結果であり、独立したconnection reuse ownerではない。

このhandoffはprimary failure classificationを所有しない。deadlineは `DEADLINE_EXCEEDED`、明示cancelは `CANCELLED`、stream-local semantic failureは既に選択済みのtaxonomyを維持し、targetのcaller-selected RST codeも変更しない。transport actionだけが新規admissionを止め、pending RST/control frameへ有限なflush機会を与えた後にconnectionをdeadへ移す。fatal I/O / nghttp2 error pathはabandonment predicateに依存せず先にconnectionをdead化するため、その後のunregister / owner clearはlocal bookkeepingだけを行う。productionはpersistent `h2_connection` のsession-lifetime trackerとclose-after-pending-flush stateをconsumerとする。raw diagnosticもiteration reset外のtrackerからsession-terminal markerへ写像し、deadlineならtarget `RST_STREAM(CANCEL)`をnonblockingでbest-effort flushして終了する。
...
`unregister_grpc_call_stream(call)` はactive登録だけを外す。nghttp2 stream user dataとactive stream listから `call` を外し、`stream_registered` と `active_stream_count` を更新するが、`stream_owner_count` は減らさない。stream close callback、GOAWAY、RST_STREAMでactive登録が外れた後でも、status resolution、result construction、resource cleanupがconnection情報を読むことがあるためである。connection-terminal判断はここで再実装しない。live local abandonmentは先行する `cancel_grpc_call_stream()` がconnection/session receive-boundary predicateを確認し、fatal error pathは先にconnectionをdead化するため、unregisterはcall pointerを外すbookkeepingに留める。
...
- local abandonment後にconnectionを再利用できるのは、connection/session receive stateがclean frame boundaryにあり、HEADERS / CONTINUATION blockも継続していない場合だけである。
- `cancel_grpc_call_stream()` はRST submit前にこのbyte-level predicateを確認し、falseならconnection lifecycleへterminal actionをhandoffする。phase stateはreuse可否を独立に決めない。
