...
| C extension memory checker | partial | ASAN/UBSANとValgrindでC unit / PHPT / HTTP/2 lifecycle fixtureがcleanであること | `./tools/test/check-c-sanitizer.sh` は専用 sanitizer PHP image、`./tools/test/check-c-valgrind.sh` はdev imageで実行する。CI matrix化は継続 |
