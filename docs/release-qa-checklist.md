...
| malformed unary frame | done | incomplete / multiple frameを `STATUS_INTERNAL` | `src/Grpc/UnaryCall.php` / `src/Grpc/Internal/Http2Transport.php` |
| cancel observable status | partial | cancel-before-wait / cancel-before-iteration は `STATUS_CANCELLED` | HTTP/2 stream cancelの追加faultは継続 |
