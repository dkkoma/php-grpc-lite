...
| gate | status | 判定 | evidence / next |
|---|---|---|---|
| `Grpc\CallCredentials::createFromPlugin()` | done | official wrapperの `call_credentials_callback` から `Grpc\Call::setCredentials()` 経由でper-call metadataへ統合 | `ext/grpc/call.c` / `tests/Integration/CallCredentialsTest.php` |
| `grpc-timeout` 8桁制限 | done | official wrapperのdeadlineをC側で `grpc-timeout` metadataとtransport deadlineへ反映 | `ext/grpc/call.c` / `tests/Integration/DeadlineTest.php` |
| malformed unary frame | done | incomplete / multiple frameを `STATUS_INTERNAL` | `ext/grpc/call.c` / `ext/grpc/transport.c` / `tests/Integration/HttpValidationTest.php` |
| cancel observable status | done | cancel-before-wait / cancel-before-iteration は `STATUS_CANCELLED` | `ext/grpc/call.c` / `tests/Integration/ControlSemanticsTest.php` |
