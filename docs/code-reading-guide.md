...
`grpc.c` の `PHP_MINIT_FUNCTION(grpc_lite)` はmodule lifecycleとINI / constants登録を担当し、PHP class登録は `src/surface.c` の `grpc_lite_register_surface_classes()` に委譲します。class/object実装とmethod tableは `src/surface.c`、official wrapperから呼ばれる `Grpc\Call::startBatch()` bridgeは `src/bridge.c` にあります。`src/surface.c` は `src/bridge.h` を通じてそのmethod implementationを参照します。
