...
- slow consumer時はPHPが次messageを要求するまで追加readを進めない。別streamのI/Oで同じconnectionを読む場合は、server streaming payload queueを4 messages / 256KiBに制限し、超過時は対象streamを `RST_STREAM(CANCEL)` で閉じる。
