...
native resourceの所有権はC extension側に閉じる。`poc_client` 相当のper-call stateが持つbody buffer、queued payload、metadata、`grpc-message` はcall完了・failure・resource destructorのいずれでも同じcleanup pathを通す。stream resource destructorは未完了streamへ `RST_STREAM(CANCEL)` を送り、channel busy状態を解除する。

## Multiplex PoC

HTTP/2 session自体は複数streamを同時にin-flightにできる。native extensionでは `nghttp2_poc_multiplex_unary()` で同一session上に複数unary streamをsubmitし、全streamが独立に完了することを検証した。
