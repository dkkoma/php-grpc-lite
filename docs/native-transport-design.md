...
HTTP/2 session自体は複数streamを同時にin-flightにできる必要がある。PoC extensionでは `nghttp2_poc_multiplex_unary()` で同一session上に複数unary streamをsubmitし、全streamが独立に完了することを検証する。

現行のpublic PHP `Grpc\` surfaceは `wait()` / `responses()` が同期blockingなので、複数RPCを同時に進めるschedulerはまだ露出しない。production化ではchannel resource上にstream tableを持ち、複数call objectから同じevent loop / poll loopを共有する形にする。
