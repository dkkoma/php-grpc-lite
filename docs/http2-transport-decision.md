...
`php-grpc-lite` の本実装transportは HTTP/2 transport の1系統にする。

ただしこれはPhase 2の設計判断であり、drop-in releaseにはまだしない。releaseには、source-built grpc extension packaging、長時間stress、large response tuning、production C extensionとしてのmemory checkを満たす必要がある。

libcurl runtime経路、transport option、環境変数によるtransport切替は持たない。extension未ロードやHTTP/2 transport errorは別経路へfallbackせず、そのRPCの失敗として扱う。
...
- source-built grpc extensionを前提にするなら、transportを1系統にした方がdrop-in surfaceの互換性とC extension hardeningへ集中できる。

## Release Default Gates

HTTP/2 transportをdrop-in release defaultにする前に満たす条件:

- source-built grpc extension packagingが通常利用できる。
- TLS / mTLS がunary / server streamingとも通る。Phase 2 HTTP/2 compatibility gateで検証済み。
...
4. **Insecure h2c benchmark path**
   - まずGo test-serverのcontrolled benchmarkでHTTP/2 MVP、ext-grpcを比較する。
   - TLS/mTLS等はMVP後のcompatibility integrationで入れる。

## Deferred

HTTP/2 transportに残すがMVP必須ではないもの:
...
- **shared event loop / multiplex scheduler**
  ...
  - 2026-05-05のHTTP/2 mux spikeでは同一HTTP/2 session上の複数active streamは実装可能と確認したが、main比でwarm unary `+13.3%`、server streaming count=1000 `+6.3%` の退行が出た。
...
transportはHTTP/2のみ。ただし、server streamingのlarge response bulk transferは、release時点でknown limitationとして扱い、実ワークロードに近いshapeでSLO内に入るかを事前確認する。

ここでのlarge判定はgRPC仕様上の制限ではなく、2026-05-04時点のcontrolled benchmarkに基づく運用閾値。

| 区分 | 目安 | 推奨 |
|---|---|---|
| small unary / small streaming | unary request/responseが数百B〜数KiB、server streamingが `1x100B` / `1x1KiB` / `1x4KiB` / `1x10KiB` 相当 | `HTTP/2` を推奨 |
| many-small streaming | 1 messageが小さい。例: `1000x100B` / `10000x100B` | HTTP/2で計測し、p99 SLOを確認 |
| single-large response | 1 messageが `>= 1MiB`、ただしmessage数は少ない | HTTP/2で計測し、p99/throughputを確認 |
...
代表例として `10x100KiB` はHTTP/2がcurlより改善するがext-grpcより遅く、`100x100KiB` はHTTP/2 stream resource surfaceがcurl/ext-grpcより明確に遅い。したがってlargeの実務閾値は「100KiB級messageが数十件以上、合計8〜10MiB級」と置く。

## Known Exceptions

- `100×100KiB` server streamingはHTTP/2 stream resource化後もext-grpcよりstream p99が遅いケースがある。small SELECT主ワークロードのrelease判断とは分け、transport selection guideのlarge bulk streaming known limitationとして扱う。
...
- **PHP 8.5 persistent curl share / libcurl runtime route**
  - target architectureはHTTP/2であり、curl routeを本実装に残さない。
...
- Large request unary MVP comparison: `docs/research/http2-transport-mvp-comparison-2026-05-03.md`
- HTTP/2 actual surface / 100×100KiB repeat: `docs/research/native-surface-repeat-2026-05-03.md`
- HTTP/2 control semantics: `docs/research/native-control-semantics-2026-05-03.md`
- Upload no-copy + poll loop: `docs/research/http2-transport-unary-large-request-conclusion-2026-05-03.md`
- HTTP/2 stream resource: `docs/research/native-stream-resource-2026-05-04.md`
- HTTP/2 slow consumer / memory surface: `docs/research/native-slow-consumer-2026-05-04.md`
- HTTP/2 compatibility gates: `docs/research/native-compatibility-gates-2026-05-04.md`
- HTTP/2 resource lifecycle cleanup: `docs/research/native-resource-lifecycle-2026-05-04.md`
...
- HTTP/2 mux / event loop spike: `docs/research/native-mux-event-loop-spike-2026-05-05.md`
