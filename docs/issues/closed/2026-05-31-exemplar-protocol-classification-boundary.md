...
- Status: Closed
- Created: 2026-05-31
- Branch: codex/exemplar-protocol-classification-boundary
- Owner: Codex
...
- 2026-05-31: `src/transport.c`、`src/status_core.c`、`src/protocol_core.c`、PHPT/PHPUnit coverageを棚卸し。
- 2026-05-31: runtime変更を入れず、current designとして `docs/design/protocol-classification-boundary.md` を追加。
- 2026-05-31: `docs/README.md` と `docs/verification/verification-matrix.md` から新design docへリンク。

## Verification

- `git diff --check`: PASS
- runtime code changeなし。docs-onlyの棚卸しなのでbenchmarkは不要。
- runtime code changeなし。PHPT/PHPUnit/domain model reviewは実装変更時のgateとしてdesign docに明記。

## Decision Log

- 2026-05-31: この作業は教材価値が高いが、protocol behaviorとhot pathに触るため計測・レビュー必須とする。
- 2026-05-31: 現行実装ではclassification flag更新と `RST_STREAM` 送信が同じhot pathに混在する。性能影響を避けるため、今回は挙動変更せずdocsで責務境界を明文化する。
- 2026-05-31: `protocol_core.c` はpure helper、`status_core.c` はstatus taxonomy、`transport.c` はcall classificationとtransport actionの現ownerとして整理した。将来refactorする場合はclassification helper導入、transport action helper化、before/after benchmark、domain model reviewを別issueで行う。
- 2026-05-31: runtime refactorは `docs/issues/open/2026-05-31-protocol-classification-runtime-boundary-refactor.md` として独立issue化した。
...
このissueではruntime変更を採用しないため、before/after計測とdomain model reviewは不要。実装変更を伴う後続refactorでは `docs/design/protocol-classification-boundary.md` のgateに従う。
