...
- `nm -D --defined-only modules/grpc.so` で export が `get_module` のみに絞られることを確認する。
...
4. Docker compose 内で build/load、symbol export、C unit、PHPT を確認する。
5. issue に検証結果と判断ログを追記して close する。

## 進捗

- 2026-06-06: issue 作成。
- 2026-06-06: `config.m4` に `-fvisibility=hidden` capability checkを追加し、production / bench buildで dynamic export が `get_module` のみに絞られることを確認した。
...
- bench build: PASS
  - `./configure --enable-grpc --enable-grpc-bench` 後の `nm -D --defined-only modules/grpc.so` は `get_module` のみ。
