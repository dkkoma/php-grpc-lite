...
`server_streaming_call_state` は、stream開始後に使わない `path` と `metadata` を保持している。現在のproduction経路では、HTTP/2 request submit後の再送は行わず、diagnostic用にも参照していないため、resource lifetimeに持ち越す必要がない。

SpannerのSELECT/DMLはserver streaming call経路を通るため、call開始時固定費として効く可能性がある。

## スコープ

- `server_streaming_call_state` から未使用の `path` と `metadata` を削除する。
