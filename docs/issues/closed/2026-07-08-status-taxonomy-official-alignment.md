...
- 2026-07-10: 第三パスレビュー（protocol-adversary `REVIEW-20260710-004` [Medium]）対応。1xx 対応（`expect_final_response`、`x-bench-early-hints` fixture、1xx PHPT、関連 docs 記述）を PR #28 から revert し、[2026-07-10-informational-1xx-response-handling](2026-07-10-informational-1xx-response-handling.md) の別 PR スコープへ移した（frame-end 判定では header callback の metadata / semantic field 反映を修復できず、initial / trailing metadata が反転する不完全な成功経路だったため）。`trailing_headers_seen` の END_STREAM ゲートと field ownership map 修正は本 PR に残置。あわせて PR 説明本文を最終挙動（Compressed-Flag=1 限定 / DATA END_STREAM 限定）に更新（compat `REVIEW-20260710-003` [Low]）。
- 2026-07-15: PR #28 マージ（merge commit e49d4be）を確認して Closed。
...
- 敵対的再レビュー対応後（2026-07-10）: 1xx / END_STREAM ゲートのケース追加込みで C unit 3本 / PHPT 17/17 / PHPUnit 31 tests / C static analysis すべてpass。※1xx ケースは一時導入 commit `375c3dd` 時点の historical result。その後 1xx は PR #28 から revert したため current テストには含まれない。
- 1xx revert 後（2026-07-10, `093b808`）: C unit 3本 / PHPT 17/17 / PHPUnit 31 tests / C static analysis すべてpass。

## Related issues (PR #28 スコープ膨張の分割)

レビュー対応で本 issue のスコープ（ステータスコードの張り替え）を超えた挙動修正が PR #28 に入り、issue を分割した（2026-07-10）。分割の境界は2種類で異なる:

- [2026-07-10-grpc-encoding-flag0-no-reject](2026-07-10-grpc-encoding-flag0-no-reject.md) — `grpc-encoding` 宣言のみで拒否しない（エラー → 成功の挙動変更）。**コードは PR #28 に同梱したまま、記録のみ別 issue**。PR #28 マージで close する。
- [2026-07-10-informational-1xx-response-handling](2026-07-10-informational-1xx-response-handling.md) — 1xx (informational) 応答対応（既存制限の修正）。一時同梱（`375c3dd`）したが第三パスレビューで不完全性が実証され、**code / fixture / PHPT / docs 記述ごと PR #28 から revert して将来の別 PR スコープへ分離**。
