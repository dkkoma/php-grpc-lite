...
## Related issues (PR #28 スコープ膨張の記録分割)

レビュー対応で本 issue のスコープ（ステータスコードの張り替え）を超えた挙動修正が PR #28 に同梱された。コードは分離せず、記録として以下へ issue 分割した（2026-07-10）:

- [2026-07-10-grpc-encoding-flag0-no-reject](2026-07-10-grpc-encoding-flag0-no-reject.md) — `grpc-encoding` 宣言のみで拒否しない（エラー → 成功の挙動変更）
- [2026-07-10-informational-1xx-response-handling](2026-07-10-informational-1xx-response-handling.md) — 1xx (informational) 応答対応（既存制限の修正）
