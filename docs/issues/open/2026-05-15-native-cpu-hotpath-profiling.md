...
- Laravel route: `/bench?action=transaction_select2_update1_insert1`
...
## 2026-05-15 計測actionの更新

`select_1row_10col` 単一SELECTは実ワークロード代表として弱いため、以後のFPM CPU問題計測では `transaction_select2_update1_insert1` を既定actionにする。このactionは、事前INSERT 1回の後、別transaction内で同じ行に対するSELECT 2回、UPDATE 1回、別行INSERT 1回を実行する。

## 2026-05-15 mixed transaction計測

条件: Cloud Spanner / FPM worker32 / CPU quota 4.0 / client concurrency 32 / `transaction_select2_update1_insert1` / startup session warmup + FPM worker HTTP warmup。

30秒sustain:

| variant | RPS | avg | p50 | p90 | p99 | avg CPU cores | throttled |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| native r1 | 24.4176 | 1,300.7ms | 1,221.5ms | 2,391.5ms | 3,504.6ms | 0.392 | 0 |
| native r2 | 24.6257 | 1,288.2ms | 1,288.1ms | 1,418.6ms | 1,580.2ms | 0.406 | 0 |
| ext-grpc | 25.5847 | 1,240.4ms | 1,248.4ms | 1,349.0ms | 1,492.8ms | 0.383 | 0 |

固定1024 successful requests:

| variant | RPS | CPU/request | avg | p50 | p90 | max |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| native | 20.7258 | 13,438.9µs | 1,530.3ms | 1,445.4ms | 1,624.1ms | 5,082.4ms |
| ext-grpc | 22.4190 | 11,970.8µs | 1,415.1ms | 1,420.1ms | 1,565.6ms | 1,756.0ms |

FastCGI sequential CPU runner:
...
- mixed transactionはSpanner write transaction待ちが支配的で、FPM CPUは0.4 core前後、throttlingなし。CPU飽和再現には向かない。
