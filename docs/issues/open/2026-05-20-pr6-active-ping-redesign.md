...
- 2026-05-20: real Cloud Spanner `SELECT 1` で10ms/100ms intervalを実測した。10msは0msに近い改善を示したが、100msはoffと同レンジで、issue #5のmitigationとしては効果が弱い。
...
- issue #5で観測された改善を再現するには、現状の単純active PINGでは `0ms` または `10ms` 程度の短いre-arm intervalが必要に見える。
