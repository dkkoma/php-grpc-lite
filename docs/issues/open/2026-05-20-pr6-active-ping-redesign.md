...
- 2026-05-20: tcpdump / grpc-lite traceでwire/control lifecycleを確認した。official ext-grpcのBDP on/offは、SELECT 1の測定RPC窓ではTCP packet形状・first response timingに大きな差が見えない。grpc-lite active PINGは、現行PR実装では後続RPCを速くするconnection-level state変化ではなく、persistent connection reuseを壊して各RPCで新規TLS connectionを作る副作用が見えた。したがって、現状のactive PING実装はmitigation候補として扱わず、以降は原因調査用の観測結果として扱う。
...
- 以前のinterval matrixではactive `0ms` / `10ms` にlatency改善が見えたが、今回のwire/control lifecycleではactive `10ms` がconnection reuseを壊している。条件差または当時の測定解釈を説明できるまで、過去の改善値はproduction mitigation根拠として使わない。
...
- grpc-liteのpersistent reuse前 preflightは、active streamがない状態でTLS readable dataが残っていると `persistent TLS connection has pending control data before reuse` としてconnectionをdrainingにする。今回のactive 10msでは、この制御系packetが測定RPC間に残り、reuse前にconnectionが捨てられている可能性が高い。ただし現時点の表はSYN/preface/stream idからの推定であり、次の検証ではreuse reject reasonを直接traceする。

判断:

- active PINGは、少なくとも現行PR実装では「同一connection上の後続RPCを速くする」実装になっていない。むしろconnection reuseを壊すため、production mitigation候補から外す。
...
- 次に必要な調査は、active PING safetyではなく、まずgrpc-liteのpersistent preflightがcontrol frameを安全にdrainできているか、またofficial C-coreがBDP/server PING/SETTINGS/WINDOW_UPDATEをRPC間でどう処理してconnection reuseを維持しているかの比較である。
