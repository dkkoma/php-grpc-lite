...
| 1 | ベンチマーク基盤整備 | レイテンシ/スループット/メモリ計測を CI で回せる | **進行中**。Docker ローカル実行・ログ保存・JSON/TSV 抽出・baseline 比較・ext-grpc 比較入口は整備済み。CI のしきい値判定は未着手 |
...
- **2026-04-27**: Phase 1 ベンチ基盤を再整理。`bench/run.sh` を通常入口として追加し、`lite` / `ext` / `compare` / `cold` / `stream` / `hot-path` の各スイートを Docker compose 内で実行して `var/bench-results/` にログ保存できるようにした。`bench/compare.sh` も互換入口として同じ実装へ委譲する。`tools/parse-phpbench-aggregate.php` で PHPBench aggregate ログを JSON/TSV に抽出し、`mode_ns`、`mem_peak_bytes`、`rstdev_percent` を機械比較できる形にした。`tools/compare-benchmark-baseline.php` と `bench/baselines/regression.json` を追加し、`BENCH_BASELINE=bench/baselines/regression.json ./bench/run.sh cold` で php-grpc-lite 自身の回帰判定を実行できるようにした。Phase 1 はローカル再現基盤まで進行、CI のしきい値判定と smoke 対象拡張は残タスク。
