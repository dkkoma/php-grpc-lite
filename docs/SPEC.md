...
- response HEADERS はrawなnghttp2 categoryではなく、call-localな `grpc_response_header_phase_state` (`AWAITING_STATUS` / `INFORMATIONAL` / `FINAL_INITIAL` / `TRAILING`)で意味を決める。production / diagnosticは同じpure phase helperを使い、valid blockではregular fieldより先に届く`:status`でphaseを確定する。wire budget計上後の `AWAITING_STATUS` でnormalまたはsilently-invalidなregular fieldを先に観測した場合は、後続`:status`を合法に置けないため、そのcallback時点で `response_header_protocol_error` / `INTERNAL` を確定する。1xx fieldはPHP-visible metadata、content-type validation、gRPC status/message/encodingに反映しない一方、pseudo-headerと反復1xxを含む全decoded response fieldは独立したcall-local wire header budgetへ計上する。1xx後のfinal `HCAT_HEADERS` はinitial metadataとして所有し、initial responseと同じvalidationを行う。`grpc-status` / `grpc-message` / `grpc-status-details-bin` はEND_STREAM付きのvalidなTrailing / Trailers-Only blockでのみstatus fieldとして採用する。これらをEND_STREAMなし `FINAL_INITIAL` で観測した場合は、status field callback時点で `invalid_grpc_status` / `UNKNOWN` を確定して当該streamへ `RST_STREAM(CANCEL)` をsubmitし、header block完了やpeerの追加frameを待たない。terminal failureを確定したblockがEND_HEADERS未完了ならinbound HPACK blockを完了できずconnection全体の継続可能性を失うため、新規stream admissionを止め、callerが選んだpending RST（`CANCEL` / `PROTOCOL_ERROR`）のflush後にconnectionをdeadへ移す。対象callはproducer固有の `UNKNOWN` / `INTERNAL` / `RESOURCE_EXHAUSTED` を維持し、既存sibling callはsocket / nghttp2 I/Oを再駆動せず `UNAVAILABLE`、follow-up RPCはfresh connectionを使う。nghttp2が拒否したresponse HEADERS sequenceやfinal response前のDATAも `response_header_protocol_error` としてHTTP status fallbackや先行 `grpc-status` より優先し `INTERNAL` へ分類する。
