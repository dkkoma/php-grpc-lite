...
- response HEADERS はrawなnghttp2 categoryではなく、call-localな `grpc_response_header_block_phase` (`AWAITING_STATUS` / `INFORMATIONAL` / `FINAL_INITIAL` / `TRAILING`)で意味を決める。nghttp2は最初のresponse HEADERSだけを `NGHTTP2_HCAT_RESPONSE` とし、1xx後のfinal response HEADERSも `NGHTTP2_HCAT_HEADERS` で通知するため、`:status` がblock先頭で届いた時点までにphaseを確定する。1xx blockは受動的に無視してfinal responseを待ち、その全fieldをmetadata、content-type validation、gRPC status/message/encoding、metadata limitの観測から隔離する。1xx後のfinal `HCAT_HEADERS` はinitial metadataとして所有し、initial responseと同じcontent-type validationを行う。
...
response metadataのinitial / trailing ownershipはresponse header-block phaseから決める。1xx informational blockのfieldはどちらのmetadataにも含めず、1xx後に `NGHTTP2_HCAT_HEADERS` で届くfinal response blockのcustom metadataはinitial metadataへ入れる。raw categoryだけで `HCAT_HEADERS` をtrailing metadataとみなさない。final response自体がtrailers-only responseの場合は、既存規則どおり同じblockのstatus metadataをtrailing ownershipとする。
...
- **2026-07-15**: response HEADERSをcall-localなsemantic phase(`informational` / `final initial` / `trailing`)で分類する方針を追加。1xx fieldをmetadata / validation / status stateから隔離し、1xx後に `NGHTTP2_HCAT_HEADERS` で届くfinal responseのmetadataをinitial ownershipとする。
