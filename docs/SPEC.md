...
- **2026-04-26**: PHPBench(`bench/`)を追加し Phase 0 完了時点のベースラインを取得・記録(`docs/benchmarks/baseline-2026-04-26.md`)。helloworld unary 240μs、Spanner unary 5.68ms、Spanner streaming 1000 行 4.12ms(行あたり marginal ~1μs)。Phase 1 以降の改善・退行検出の原点として固定。
- **2026-04-26**: ext-grpc 比較環境を構築(`Dockerfile.ext-grpc` + `bench-comparison/` 独立 Composer プロジェクト)。同一 fixture / 同一サーバ / 同一 PHPBench で A/B 計測した結果(`docs/benchmarks/comparison-ext-grpc-2026-04-26.md`):helloworld unary は ext-grpc が 2.6× 速(persistent channel 効果)、Spanner unary は php-grpc-lite が 2.4× 速、**Spanner streaming は php-grpc-lite が 1.2-1.5× 速**(行あたり marginal cost で 1.7× 速)。Phase 1 の改善方向(接続プーリング)が定量化された。
