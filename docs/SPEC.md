...
- response HEADERS はrawなnghttp2 categoryではなく、call-localな `grpc_response_header_phase_state` (`AWAITING_STATUS` / `INFORMATIONAL` / `FINAL_INITIAL` / `TRAILING`)で意味を決める。production / diagnosticは同じpure phase helperを使い、`:status` がblock先頭で届いた時点までにphaseを確定する。1xx fieldはPHP-visible metadata、content-type validation、gRPC status/message/encodingに反映しない一方、pseudo-headerと反復1xxを含む全decoded response fieldは独立したcall-local wire header budgetへ計上する。1xx後のfinal `HCAT_HEADERS` はinitial metadataとして所有し、initial responseと同じvalidationを行う。`grpc-status` / `grpc-message` / `grpc-status-details-bin` はEND_STREAM付きのvalidなTrailing / Trailers-Only blockでのみstatus fieldとして採用する。これらをEND_STREAMなし `FINAL_INITIAL` で観測した場合は、status field callback時点で `invalid_grpc_status` / `UNKNOWN` を確定して当該streamへ `RST_STREAM(CANCEL)` をsubmitし、header block完了やpeerの追加frameを待たない。END_HEADERS未完了ならinbound HPACK blockを完了できずconnection全体の継続可能性を失うため、新規stream admissionを止め、pending CANCELのflush後にconnectionをdeadへ移す。対象callは `UNKNOWN` を維持し、既存sibling callはsocket / nghttp2 I/Oを再駆動せず `UNAVAILABLE`、follow-up RPCはfresh connectionを使う。nghttp2が拒否したresponse HEADERS sequenceやfinal response前のDATAは `response_header_protocol_error` としてHTTP status fallbackや先行 `grpc-status` より優先し `INTERNAL` へ分類する。
