...
HTTP/2 transportをrelease default候補へ近づけるため、libcurl経路で既に持っている互換性・制御系のうち、HTTP/2 surfaceで不足していた部分を埋める。
...
- `Http2TransportControlTest`: 24 tests / 74 assertions / 1 skipped
...
HTTP/2 transportの主要compatibility gateは、Phase 2 MVP surface上では通った。
