...
- slow consumer limitation check
  - actual native surface用のslow consumer runnerを追加。
  - 現MVP native wrapperはbatch drain後にyieldするため、真のbackpressure検証ではなく制限事項の確認である。
...
BENCH_TAG=20260503-native-slow-consumer-smoke STREAMS=3 MESSAGE_COUNT=20 PAYLOAD_BYTES=100 SLEEP_US=1000 bench/phase2/check-native-slow-consumer.sh
docker compose run --rm dev php tools/phase2/slow-consumer-surface.php --output=var/bench-results/phase2-curl-slow-consumer-20260503-smoke.json --streams=3 --message-count=20 --payload-bytes=100 --sleep-us=1000 --transport=curl
```
