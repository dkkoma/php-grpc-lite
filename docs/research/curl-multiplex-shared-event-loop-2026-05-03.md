...
native nghttp2 transportへ直接入る前に、まず既存ext-curlでできる範囲として以下を試した。
...
これはlibcurl経由のPoCであり、native nghttp2実装ではない。またprotobuf decode/yieldは行わず、gRPC frameのmessage countだけを数える。目的はAPI実装候補ではなく、shared event loop / multiplexの性質を見ること。
...
## native transportへの含意

native nghttp2 transportでchannel shared event loopを入れるなら、目的は単一call latency削減ではなく、以下になる。
...
1. unary large request / server streaming large responseで確認済みのnative transport pathを本実装設計へ落とす。
...
このPoCはlibcurl上のmultiplexであり、native nghttp2で複数active streamを直接管理したわけではない。
