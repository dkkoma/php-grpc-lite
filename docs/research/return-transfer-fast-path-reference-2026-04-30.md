...
| `tools/benchmark/payload-unary.php` | `--return-transfer-fast-path` を追加 |
| `bench/run.sh` | `payload-unary-return-transfer-fast-path` suite を追加 |
...
| 実行入口 | `./bench/run.sh payload-unary-diagnostic-cached` / `payload-unary-return-transfer-fast-path` |
