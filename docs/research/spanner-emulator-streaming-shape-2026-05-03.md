...
Spanner emulator の `ExecuteStreamingSql` が、結果セット形状に応じてどのような `PartialResultSet` message列を返すかを実測した。あわせて、10列テーブルに対する1件DML insert / update / deleteを、明示的な `BeginTransaction` → `ExecuteSql` DML → `Commit` のunary flowとして実測した。
...
Each operation uses an explicit read-write transaction:

```text
BeginTransaction -> ExecuteSql(DML) -> Commit
```

| operation | rpc | request bytes | response bytes | row count |
|---|---|---:|---:|---:|
| insert | BeginTransaction | 92 | 18 | |
| insert | ExecuteSqlDml | 355 | 8 | 1 |
| insert | Commit | 106 | 14 | |
| update | BeginTransaction | 92 | 18 | |
| update | ExecuteSqlDml | 327 | 8 | 1 |
| update | Commit | 106 | 14 | |
| delete | BeginTransaction | 92 | 18 | |
| delete | ExecuteSqlDml | 144 | 8 | 1 |
| delete | Commit | 106 | 14 | |
...
- DML本体はinsert/updateでもrequest 300B台、response 8Bだった。deleteはrequest 144B。Begin/Commitも100B前後のrequestと20B未満のresponseで、DML flowは「小さいunaryを3回」の形になる。
