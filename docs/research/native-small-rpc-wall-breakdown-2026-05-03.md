...
Spanner 主用途に近い small unary / small server streaming で、`php-grpc-lite` の libcurl 経路、nghttp2 PoC 経路、公式 `ext-grpc` を比較した。
...
| case | total p50 | HTTP/2 transport p50 | connect p50 | initial send p50 | recv loop p50 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
- `Http2Transport::unarySimple()` は target ごとに channel を保持して再利用する
...
`BENCH_TAG=20260503-spanner-dml-unary-persistent bench/compare-spanner-dml-unary-shape.sh`
...
`INCLUDE_POC=0 WARMUP_STREAMS=3 BENCH_TAG=20260503-small-select-warm3-smoke bench/compare-small-select-streaming.sh`
...
Phase 2 MVP の設計上は、HTTP/2 transport を採用するなら C 拡張内に Channel lifetime の HTTP/2 session を持つ必要がある。libcurl 経路を残す場合も、性能上の主経路ではなく安全・安定経路として扱うのが妥当。
