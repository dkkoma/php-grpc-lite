...
HTTP/2 transportをproduction C extensionへ寄せる前に、PoC extension内のresource ownershipとfailure pathを見直す。
...
`Http2TransportControlTest::testNativeStreamResourceDestructReleasesChannelBusyState`
...
- `Http2TransportControlTest`: 25 tests / 78 assertions / 1 skipped
...
```bash
BENCH_TAG=20260504-native-lifecycle-stress ITERATIONS=100 MESSAGE_COUNT=20 PAYLOAD_BYTES=1024 ./bench/check-native-lifecycle-stress.sh
VALGRIND=1 BENCH_TAG=manual-valgrind ITERATIONS=5 ./bench/check-native-lifecycle-stress.sh
```
...
```bash
BENCH_TAG=20260504-native-lifecycle-ownership ITERATIONS=100 MESSAGE_COUNT=20 PAYLOAD_BYTES=1024 ./bench/check-native-lifecycle-stress.sh
```
