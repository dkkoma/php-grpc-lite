...
server streaming native surfaceを `unaryBatch()` のbatch drain後yieldから、C stream resourceをPHP Generatorがpullする形へ切り替える。
...
- `NativeTransportControlTest`: 13 tests / 38 assertions / 1 skipped
...
| 100x100KiB stream | 5324.9 / 11016.7 μs | 9472.6 / 15326.9 μs | 5408.6 / 9540.7 μs | native surfaceが悪化、要調査 |
...
- large / many-large responseでは、PoC direct/compactに比べてnative surfaceがまだ遅いケースがある。特に `100x100KiB` はstream resourceのpull粒度、queued payload、PHP deserialize/yield、WINDOW_UPDATE進行のどこで差が出ているかを追加分解する必要がある。
