...
- 次に進めるなら、C extension を本体化する前に `php-grpc-lite` の PHP userland 側で削れる固定費を先に潰し、その後に「HTTP/2 transport を入れるならどの surface を C に落とすか」を決める。
...
| TLS/mTLS版HTTP/2 transport | 実運用に近いtransport overheadを見る | TLS handshake除外のwarm p50/p99、large request | PoC実装量が増える。Phase 2 transport判断では後回しでよい |
