...
- PHP/libcurl公開APIで分解できる境界は、`posttransfer`、`starttransfer`、server InPayload、curl trace DATA_OUT まで。これ以上、libcurl内のnghttp2 flow-control、socket write、WINDOW_UPDATE待ちを定量分解するには、libcurl/nghttp2をinstrumentしたビルド、またはC拡張/native transport側の計測が必要。
...
3. これ以上 `libcurl/nghttp2内部` を掘るなら、PHP userlandではなく `_research` のlibcurl/nghttp2 source readingか、instrumented libcurl build / native transport PoCで見る。
