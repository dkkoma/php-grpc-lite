...
| `partial-headers-payload-deadline` | live unary streamへpadding付きHEADERSの9-byte frame headerとpad-length 1 byteだけを送り、HPACK field bytesをすべて保留する。`on_begin_headers_callback()` / field callback前のdeadline abandonmentが `DEADLINE_EXCEEDED` + exact `RST_STREAM(CANCEL)`を維持しつつconnectionをterminal化し、authority-keyed follow-upをfresh connectionへ移すことを検証する |
| `clean-headers-boundary-deadline` | complete final initial HEADERSを送ってframe boundaryへ戻った後はpeerをsilentに保つ。deadline abandonmentがexact `RST_STREAM(CANCEL)`を送ってもreceive byte-boundary predicateはreuseを許可し、次のcontrolを同じconnectionへsubmitできるpositive control |
| `require-prior-clean-boundary-cancel` | 同じconnection上の直前の `clean-headers-boundary-deadline` streamについてexact `RST_STREAM(CANCEL)` を受信済みの場合だけgRPC OKを返す。partial-frame terminal caseとclean-boundary reuse caseを同じstatusだけでは通せないよう分離する |
| `partial-data-compressed-message` | complete initial HEADERS後、payload length 6のDATA frame headerとCompressed-Flag付きgRPC header 5 byteだけを送り、最後の1 byteを保留する。DATA callback内のsemantic `INTERNAL` + exact `RST_STREAM(CANCEL)`がprimary detailsを維持し、follow-up前にconnectionをdead化することを検証する |
| `late-partial-frame-header-after-clean-cancel` / `await-late-partial-frame-header` | clean initial HEADERS後のdeadline CANCELをpeerが観測してから次DATA frame headerの8 byteだけを送り、別authorityのbarrierが`TIOCOUTQ == 0`を確認する。元clientのinactive preflightがexact 8 byteをdrain後にdirty predicateで旧connectionをdead化し、fresh connectionへfallbackすることを検証する |
