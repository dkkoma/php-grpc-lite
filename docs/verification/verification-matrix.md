...
| Informational 1xx isolation / malformed taxonomy / terminal status-field action | 複数1xxのisolation、malformed sequence、END_STREAMなしfinal initialの `grpc-status` / `grpc-message` / `grpc-status-details-bin` を2秒guard内にUNKNOWN + exact `RST_STREAM(CANCEL)`、same-connection follow-upまでcovered | unaryと同じ3 status fields、finite guard、exact CANCEL、follow-upまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_response_header_phase.c`。diagnosticは3 fieldともfailed-not-timedoutかつstream error code 8を固定 |
