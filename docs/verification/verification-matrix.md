...
| Informational 1xx isolation / exhaustive header routing / malformed taxonomy / terminal status-field action / Trailers-Only metadata ownership | 複数1xx isolation、normal / recoverably-invalid regular、strict-rejected pseudo / regular、unknown-class fail-closed、3 status field、informational END_STREAM / non-terminal trailingのEND_HEADERS未完了blockをprimary taxonomy + exact RSTで有限終了、nghttp2-owned RSTの非重複、未完了connectionのcontrol flush後terminal化、closed streamのlate incomplete HEADERS後も完了callのOK維持 / fresh follow-up、phase開始後のlive fragmented block abandonment、phase開始前のpartial HEADERS payload deadlineをtarget taxonomy + exact CANCEL + dead connection / fresh follow-upで固定、clean frame boundary deadlineはsame-connection reuseを固定、partial DATA callback内semantic RSTはfollow-up前のdead化を固定、clean cancel後に遅着した8-byte frame header prefixはinactive preflightでdead化 / fresh fallbackを固定、既存siblingのtarget RST後追加DATAなしUNAVAILABLE / fresh follow-up、terminal message-only / details-only blockのbefore-after metadataをtrailingへ統一までcovered | unaryと同じfield-class × phase route、strict rejection / TEMPORAL observer、status fields、incomplete-block action、late unowned HEADERSのconnection-terminal action、connection/session receive byte-boundary predicate、partial DATA semantic RST、fresh follow-up、message-only / details-only metadata ownershipまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/phpt/044-terminal-status-rst-flush-failure.phpt`, `tests/unit/test_response_header_phase.c`, `tests/unit/test_transport_core.c`。C unitはSTATUS / REGULAR / INVALID_REGULAR / REJECTED × NONE / AWAITING_STATUS / INFORMATIONAL / FINAL_INITIAL / TRAILINGとunknown default、connection-scopeのunowned incomplete HEADERS predicate、partial frame header / payload・HEADERS / CONTINUATION継続・clean boundaryをtable-driveする。PHPT 042はapplication callback前のpartial HEADERS payload deadlineをfresh-preface follow-upで、complete HEADERS後のclean-boundary deadlineをsame-connection exact CANCEL gateで識別する。partial DATAはsemantic details / exact CANCEL / DATA frame未完了 / follow-up前dead化を、late frame-header prefixは別authority barrierの`TIOCOUTQ == 0`後にexact 8-byte preflight drain / dead old connection / fresh prefaceを識別する。既存のlive fragmented informational HEADERS中のunary deadlineとtrailing HEADERS中のserver streaming explicit cancelも維持する。multiplexではpre-target-RST DATAをpositive setupとして要求してclient trace / peer marker双方でpost-target-RST DATA不在、target RST→`rpc.end`の固定grace内終了を確認する。diagnosticは同じshared route / connection-session byte trackerとdefault-blocking incomplete batch後の実fd `O_NONBLOCK` を確認する |
| Informational wire-header resource budget / production-bench parity | pre-final entry / byte、valid / invalid regular field、pseudo / custom累積、専用RESOURCE_EXHAUSTED details、exact CANCEL、complete blockのsame-connection reuse、invalid callback TEMPORAL cutoff 128、invalid-field END_HEADERS未完了budget blockのconnection terminal化 / fresh follow-upまでcovered | unaryと同じpre-final failure owner、details、cancel、invalid callback cutoff、invalid-field incomplete-block producerまでcovered | `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_transport_core.c`。productionはcomplete invalid blockのtrace 128件とincomplete invalid blockのterminal/fresh lifecycleを固定する。diagnosticはcallback count 128、default 64KiB rejection、entry / byte counter resetに加え、default-blocking batchのterminal action後に実fdが `O_NONBLOCK` であることを確認する。multiplexはtarget RST後のsibling DATAをclient trace / peer marker双方で拒否する |
...
| transport入力limit / authority validation / receive byte boundary | input / connection state only | input / connection state only | `tests/unit/test_transport_core.c`。partial 9-byte frame header、partial HEADERS / DATA payload、zero-length frame、HEADERS→CONTINUATION block、複数frame / chunk splitとreuse-after-abandonment predicateをpure unitで固定する |
