...
| Informational 1xx isolation / malformed taxonomy / terminal status-field action / Trailers-Only metadata ownership | 複数1xx isolation、malformed sequence、3 status fieldのnon-terminal / END_HEADERS未完了blockをUNKNOWN + exact CANCELで終了、未完了connectionのCANCEL後terminal化、既存siblingの追加I/OなしUNAVAILABLE / fresh follow-up、terminal message-only / details-only blockのbefore-after metadataをtrailingへ統一までcovered | unaryと同じ3 status fields、incomplete block action / fresh follow-up、message-only / details-only metadata ownershipまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/phpt/044-terminal-status-rst-flush-failure.phpt`, `tests/unit/test_response_header_phase.c`。C unitはblock-local candidate/reset、diagnosticは3 fieldのshared callback actionを固定 |
| Informational wire-header resource budget / production-bench parity | pre-final entry / byte、valid / invalid regular field、pseudo / custom累積、専用RESOURCE_EXHAUSTED details、exact CANCEL、same-connection reuse、invalid callback TEMPORAL cutoff 128までcovered | unaryと同じpre-final failure owner、details、cancel / reuse、invalid callback cutoffまでcovered | `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_transport_core.c`。productionはtrace 128件、diagnosticはcallback count 128、default 64KiB rejection、entry / byte counterそれぞれのiteration resetを固定 |
