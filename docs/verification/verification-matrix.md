...
| Informational 1xx isolation / exhaustive header routing / malformed taxonomy / terminal status-field action / Trailers-Only metadata ownership | 複数1xx isolation、normal / recoverably-invalid regular、strict-rejected pseudo / regular、unknown-class fail-closed、3 status field、informational END_STREAM / non-terminal trailingのEND_HEADERS未完了blockをprimary taxonomy + exact RSTで有限終了、nghttp2-owned RSTの非重複、未完了connectionのcontrol flush後terminal化、既存siblingのtarget RST後追加DATAなしUNAVAILABLE / fresh follow-up、terminal message-only / details-only blockのbefore-after metadataをtrailingへ統一までcovered | unaryと同じfield-class × phase route、strict rejection / TEMPORAL observer、status fields、incomplete-block action、fresh follow-up、message-only / details-only metadata ownershipまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/phpt/044-terminal-status-rst-flush-failure.phpt`, `tests/unit/test_response_header_phase.c`。C unitはSTATUS / REGULAR / INVALID_REGULAR / REJECTED × NONE / AWAITING_STATUS / INFORMATIONAL / FINAL_INITIAL / TRAILINGとunknown defaultをtable-driveする。PHPT 042はapplication callbackを迂回するstrict rejectionも `INTERNAL` + exact PROTOCOL_ERROR + terminal connection / fresh follow-upで固定し、multiplexではpre-target-RST DATAをpositive setupとして要求してclient trace / peer marker双方でpost-target-RST DATA不在、target RST→`rpc.end`の固定grace内終了を確認する。diagnosticは同じshared routeとdefault-blocking incomplete batch後の実fd `O_NONBLOCK` を確認する |
