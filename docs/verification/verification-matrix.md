...
| Informational 1xx isolation / malformed taxonomy / wire budget / production-bench parity | valid / invalid regular field、pseudo / custom entry・byte累積、exact `RST_STREAM(CANCEL)`、status-details terminal gateまでcovered | valid / invalid regular field、pseudo / custom entry・byte累積、exact `RST_STREAM(CANCEL)`、status-details terminal gateまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_response_header_phase.c`。diagnosticはvalid 1xxの2-iteration reset、default entry / byte budget rejection、foreign pushed-stream RSTのownershipも固定 |
