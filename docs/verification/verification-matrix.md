...
| Informational 1xx isolation / malformed taxonomy / terminal status-field action / Trailers-Only metadata ownership | 複数1xx isolation、malformed sequence、normal / invalid regular-before-`:status` fragmented block、3 status field、informational END_STREAM / non-terminal trailingのEND_HEADERS未完了blockをprimary taxonomy + exact RSTで有限終了、未完了connectionのcontrol flush後terminal化、既存siblingのtarget RST後追加DATAなしUNAVAILABLE / fresh follow-up、terminal message-only / details-only blockのbefore-after metadataをtrailingへ統一までcovered | unaryと同じregular-before-status normal / invalid callback、status fields、incomplete-block action、fresh follow-up、message-only / details-only metadata ownershipまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/phpt/044-terminal-status-rst-flush-failure.phpt`, `tests/unit/test_response_header_phase.c`。PHPT 042はregular-before-statusを `INTERNAL` + exact PROTOCOL_ERRORで固定し、multiplexではpre-target-RST DATAをpositive setupとして要求してclient trace / peer marker双方でpost-target-RST DATA不在、target RST→`rpc.end`の固定grace内終了を確認する。diagnosticはdefault-blocking incomplete batch後の実fd `O_NONBLOCK` を `incomplete_header_fd_nonblocking=true` で確認する |
