...
| Informational 1xx isolation / malformed taxonomy / terminal status-field action / Trailers-Only metadata ownership | 複数1xx isolation、malformed sequence、3 status fieldとinformational END_STREAM / non-terminal trailingのEND_HEADERS未完了blockをprimary taxonomy + exact RSTで終了、未完了connectionのcontrol flush後terminal化、既存siblingのtarget RST後追加DATAなしUNAVAILABLE / fresh follow-up、terminal message-only / details-only blockのbefore-after metadataをtrailingへ統一までcovered | unaryと同じstatus fields、informational / trailing incomplete-block action、fresh follow-up、message-only / details-only metadata ownershipまでcovered | `tests/phpt/022-error-and-http-validation.phpt`, `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/phpt/044-terminal-status-rst-flush-failure.phpt`, `tests/unit/test_response_header_phase.c`。PHPT 042はpre-target-RST DATAをpositive setupとして要求し、client trace / peer markerの双方でpost-target-RST DATA不在、client traceでtarget RST→`rpc.end`の固定grace内終了を確認する。C unitはblock-local candidate/reset、diagnosticはshared incomplete-block classificationと有限終了を固定 |
| Informational wire-header resource budget / production-bench parity | pre-final entry / byte、valid / invalid regular field、pseudo / custom累積、専用RESOURCE_EXHAUSTED details、exact CANCEL、same-connection reuse、invalid callback TEMPORAL cutoff 128、END_HEADERS未完了budget blockのconnection terminal化までcovered | unaryと同じpre-final failure owner、details、cancel / reuse、invalid callback cutoff、incomplete-block actionまでcovered | `tests/phpt/042-informational-1xx-adversarial.phpt`, `tests/phpt/043-informational-1xx-bench-parity.phpt`, `tests/unit/test_transport_core.c`。productionはtrace 128件、target RST後のsibling DATA `frame_out`不在、peer markerのpost-target-RST DATA不在、diagnosticはcallback count 128、default 64KiB rejection、entry / byte counterそれぞれのiteration resetを固定 |
