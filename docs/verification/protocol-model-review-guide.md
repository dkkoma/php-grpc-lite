...
| Wrapper adapter | official wrapper batch opをgRPC callへ写像、status/metadataをPHP shapeへ変換 | HTTP/2 connection lifecycleの判断 |
...
- Wrapper adapterがprotocol failure一般をconnection discardへ畳んでいないか。
