...
- Fix summary: PR全体の最終代表ベンチとして、merge-base `b5592d4` とHEAD `41855e1` を同一Docker compose project / 同一test-serverで `spanner-shape --calls=1000 --warmup-calls=50` により比較し、結果をwork-plan issueへ記録した。追加確認では `select_1row_10col_streaming` のp50 / p90 / p99を直近2ペアで確認し、p50 / p90はほぼ同等、p99はrun間揺れが大きくHEAD悪化方向のrunもあるため、判断は「p50/p90で明確な悪化なし、p99 tailは継続観測」とした。
- Fix commit: `419db7a`
- Verification: `pr-whole-before-20260530-spanner-shape`, `pr-whole-before2-20260530-spanner-shape`, `pr-whole-after-20260530-spanner-shape`, `pr-whole-after2-20260530-spanner-shape` をOTEL summaryで確認。追加で `select_1row_10col_streaming` のp50/p90/p99確認として `pr-whole-before3-20260530-spanner-shape`, `pr-whole-before4-20260530-spanner-shape`, `pr-whole-after3-20260530-spanner-shape`, `pr-whole-after4-20260530-spanner-shape` を確認。
