...
- Phase 5 current uncommitted layout move: C source/internal headers under `src/`, diagnostic bench files under `src/diagnostic`, repository root retaining `config.m4` and `main.c`
...
- Evidence: `src/diagnostic/bench.c:6` says the file is included from `main.c`; `main.c:32`-`main.c:36` only defines the empty production `grpc_lite_functions` table when bench is disabled; `config.m4:15`-`config.m4:17` owns the bench source inclusion boundary; `src/diagnostic/bench.c:2235`-`src/diagnostic/bench.c:2242` defines the bench function table for bench builds.
- Expected model: Diagnostic / benchmark entrypoints should be documented as a separate bench-only compilation unit selected by build configuration. Production `main.c` remains the module root for class/INI/module lifecycle and must not appear to own or include diagnostic transport entrypoint implementation.
...
- `config.m4:15` keeps the production source list to `main.c` plus `src/protocol_core.c`, `src/status_core.c`, `src/transport_core.c`, `src/surface.c`, `src/transport.c`, `src/unary_call.c`, `src/server_streaming_call.c`, and `src/bridge.c`.
...
- `main.c:5`-`main.c:6` includes only `src/surface.h` and `src/transport.h`; it does not include moved `.c` files.
- `main.c:32`-`main.c:36` keeps the production PHP function table empty when bench entrypoints are not enabled.
...
- Reviewed the current uncommitted Phase 5 diff, moved source/header layout, `config.m4`, `main.c`, `src/diagnostic/bench.c`, `src/diagnostic/diagnostic.h`, representative production headers, `docs/SPEC.md`, `docs/code-reading-guide.md`, `docs/protocol-model-review-guide.md`, and prior review issue style.
