...
grpc_lite_phpt_assert_same(18, count($incompleteRstFrames), 'incomplete block traces contain one RST_STREAM each');
$incompleteRstCodes = array_count_values(array_map(
    static fn (array $frame): mixed => $frame['error_code'] ?? null,
    $incompleteRstFrames,
));
grpc_lite_phpt_assert_same(8, $incompleteRstCodes[1] ?? 0, 'incomplete malformed blocks emit PROTOCOL_ERROR');
grpc_lite_phpt_assert_same(10, $incompleteRstCodes[8] ?? 0, 'incomplete status and budget blocks emit CANCEL');
grpc_lite_phpt_assert_same(36, $incompleteConnectionPrefaces, 'incomplete blocks quarantine eighteen connections before follow-up');
