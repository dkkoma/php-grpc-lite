...
docker compose run --rm dev php -d extension=/workspace/ext/grpc/modules/grpc.so \
...
docker compose run --rm dev-zts php -d extension=/workspace/ext/grpc/modules/grpc.so \
    tools/benchmark/zts-parallel-call-path.php \
    --mode=thread \
    --workers="$workers" \
    --calls="$calls" \
