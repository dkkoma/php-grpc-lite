...
docker compose run --rm dev sh -lc     "php -d extension=/workspace/modules/grpc.so tools/benchmark/slow-consumer-surface.php --implementation=php-grpc-lite --autoload=vendor/autoload.php --streams=$streams --message-count=$message_count --payload-bytes=$payload_bytes --sleep-us=$sleep_us --native-response-mode=stream"
