...
for ($i = 0; $i < $iter; $i++) {
    $t0 = hrtime(true);
    $db->runTransaction(function ($tx) use ($i) {
        $tx->execute('SELECT @i', ['parameters' => ['i' => $i]])->rows()->current();
        $tx->commit();
    });
...
printf(
    "ext.grpc=%s  ext.spanner=%s  iter=%d  mean=%.0fus  p50=%.0f  p90=%.0f  p99=%.0f  min=%.0f  max=%.0f\n",
    phpversion('grpc') ?: 'n/a',
